...
public class BookDAO implements DAO<Book> {
  ...
  private Predicate parseFilter(String[] filter, CriteriaBuilder cb, Root<Book> c) throws IllegalArgumentException {
    ...
    if(filter.length > 0){
      for(int i=0; i < filter.length; i++) {
        if(filter[i].equals(",")){
          return cb.or(parseFilter(subarray(filter, 0, i), cb, c), parseFilter(subarray(filter, i+1, filter.length), cb, c));
        }
      }
  
      for(int i=0; i < filter.length; i++) {
        if(filter[i].equals(";")){
          return cb.and(parseFilter(subarray(filter, 0, i), cb, c), parseFilter(subarray(filter, i+1, filter.length), cb, c));
        }
      }
  ...
  private Predicate base(String[] filter, CriteriaBuilder cb, Root<Book> c) throws IllegalArgumentException, NoSuchFieldException{
    if(filter[1].equals(">")){
