...
/**
 * Review controller for handling review creation. Reviews are gotten on book return in book
 * controller
 */
@RestController
    ...
    if (auth != null) {
      ...
      if (principal instanceof UserDetails) {
        ...
        if (review.getUser().getId()
            != user.getId()) { // check if review is owned by logged in user
          throw new ResponseStatusException(
              HttpStatus.BAD_REQUEST, "This review belongs to another user.");
        }
